...
def train_eval(ds_raw, train_test_ratio, rank, num_tests):
    ...
    for test in range(num_tests):
        ...
        totals = np.zeros((len(users), len(items)))
        counts = np.zeros((len(users), len(items)))
        for r in train:
            i = int(r[0])
            j = int(r[1])
            totals[i][j] += r[3]
        ...
        matrix_train = np.zeros((len(users), len(items)))
        for (i,j) in np.argwhere(counts > 0):
            matrix_train[i][j] = totals[i][j]/counts[i][j]
        density = np.sum(counts > 0) / (len(users) * len(items))
        print(f'\t2D test samples: {np.sum(counts>0)}')
        print(f'\tDensity of train matrix: {density}')
        
        U, sigma, Vt = svds(matrix_train, k=rank)
        sigma = np.diag(sigma)
...
if __name__ == "__main__":
    ...
    from prepare_incarmusic import ds_raw as ds_music
    from prepare_frappe import ds_raw as ds_frappe
    #from prepare_tripadvisorv2 import ds_raw as ds_hotel
    print("DePaulMovie")
    train_eval(ds_movie, 0.95, 30, 5)
    #print("TripAdvisorV2")
    #train_eval(ds_hotel, 0.9, 30, 5)
    print("InCarMusic")
    train_eval(ds_music, 0.95, 30, 5)
    print("Frappe")
    train_eval(ds_frappe, 0.95, 30, 5)
